...
(() => {
  ...
  function turnNavWhite() {
    ...
    const sections = gsapWithCSS.utils.toArray("[data-dark-header]");
  ...
  // src/utils/updateFooterYear.ts
  function updateFooterYear() {
    document.addEventListener("DOMContentLoaded", () => {
      const yearEl = document.querySelector("#year");
      if (!yearEl)
        return;
      yearEl.textContent = (/* @__PURE__ */ new Date()).getFullYear().toString() + " ";
    });
  }
  ...
  function navbarAnim() {
    const showNav = gsapWithCSS.from(".navbar-wrapper", {
      yPercent: -100,
      paused: true,
      duration: 0.25
    }).progress(1);
